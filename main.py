...
def passwordMatch(password, verify):
    if password == verify:
        return True
    else:
        return False
...
# DONE - TODO 3: Add "/login" GET and POST routes.
@app.route('/login', methods=['GET', 'POST'])
def login():
    return render_template('login.html')
# DONE - TODO 4: Create login template with username and password.
...
def register():
    if request.method == 'POST':
        ...
        # DONE - TODO 1: validate that form value of 'verify' matches password
        if not passwordMatch(password, verify):
            flash('Your passwords do not match!')
            return redirect('/register')
        # DONE - TODO 2: validate that there is no user with that email already
...
# DONE 5: modify this function to rely on a list of endpoints that users can visit without being redirected.
